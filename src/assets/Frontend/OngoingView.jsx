...
const OngoingView = () => {
  ...
  const fetchImages = async () => {
    try {
      const response = await fetch('https://backend.akashbariresort.com/api/peoject-view');
      if (!response.ok) {
        throw new Error('Failed to fetch images');
      }
      const data = await response.json();
      
      // Extract image_urls from the response
  ...
  if (loading) {
    return (
      <div className="container my-5 text-center">
        ...
        <p className="mt-2" style={{ color: brandColor }}>Loading images...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="container my-5 text-center">
        <p style={{ color: 'red' }}>Error loading images: {error}</p>
        <button 
          className="btn" 
    ...
    return (
      <div className="container my-5 text-center">
        <p style={{ color: brandColor }}>
          Not enough images available. Need at least 6 images. 
          Currently have {images.length} images.
        </p>
      </div>
    );
  }
      ...
      <div className="container my-5 project-view-container">
        ...
        <style>
          {`
            ...
            /* Desktop & Large Screens Default Height */
            .custom-project-row {
              height: 500px;
            }
            .big-img-col {
            ...
            /* Medium Screens or Tablets (769px - 1024px) */
            @media (min-width: 769px) and (max-width: 1024px) {
              .custom-project-row {
                height: 400px;
              }
