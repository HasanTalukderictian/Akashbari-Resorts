...
//                                     Become a proud <strong>property owner </strong> by investing in various projects of Akashbari Hotels & Resorts.
//                                 </p>
...
const HappyClient = () => {
    ...
    // Refs + visibility state for the scroll-triggered slide-in animation
    ...
    // Trigger the slide-in animation whenever a column scrolls into view,
    // and reset it when it scrolls out so it can play again next time.
    useEffect(() => {
        const makeObserver = (setInView) =>
            new IntersectionObserver(
                ([entry]) => setInView(entry.isIntersecting),
                { threshold: 0.2 }
            );

        const leftObserver = makeObserver(setLeftInView);
        const rightObserver = makeObserver(setRightInView);

        if (leftColRef.current) leftObserver.observe(leftColRef.current);
        if (rightColRef.current) rightObserver.observe(rightColRef.current);
    ...
    return (
        <section style={{ backgroundColor: '#ffffff' }}>
            <div style={heroSectionStyle}>
                <div className="container">
                    <div className="row align-items-center">
                        {/* Left Column - Text Content (slides in from the left on scroll) */}
                        <div ref={leftColRef} className={`col-md-6 anim-slide-left ${leftInView ? 'in-view' : ''}`}>
                            {/* Desktop & Mobile - Different font sizes */}
                            <span 
                                className="fw-bold mb-3 d-inline-block brand-text" 
                                style={{ 
                        ...
                        {/* Right Column - Features (slides in from the right on scroll) */}
                        <div ref={rightColRef} className={`col-md-6 anim-slide-right ${rightInView ? 'in-view' : ''}`}>
                            <div className="ps-md-4" style={{ 
                                borderLeft: '1px solid rgba(0,0,0,0.1)',
                                paddingLeft: '15px',
                                paddingRight: '15px'
