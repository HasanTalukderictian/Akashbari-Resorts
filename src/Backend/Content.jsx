...
const Content = ({ theme: propsTheme }) => {
    ...
    // --- UPDATED: Fetch individual feature by ID (for editing single feature) ---
    const viewFeature = async (featureId) => {
    ...
    // --- UPDATED: View ALL features for a category with proper IDs ---
    const viewFeatureCategory = async (categoryName) => {
        ...
        // Fetch all features for this category from the API to get their actual IDs
        // Option 1: Use the view-about-feature API for each feature (if you have individual endpoints)
        // Option 2: Use the groupedFeatures data but we need to fetch actual IDs
        
        // Since we don't have individual IDs from groupedFeatures,
        // we need to fetch each feature individually to get its ID
        try {
            const headers = getAuthHeaders();
            // Fetch all features for this category from the API
            // Assuming your API has a way to get features by category
            // For now, we'll use the groupedFeatures data and assign temporary IDs
            // But the correct approach is to fetch from API
            
            // Let's try to fetch features by category from the API
            const res = await fetch(`${API_BASE_URL}/get-about-features`, { headers });
            const result = await res.json();
            ...
            if (result.status === 'success') {
                ...
                if (categoryDataWithIds && categoryDataWithIds.details) {
                    // We need to fetch each feature's actual ID
                    // For now, we'll use the featureTitle.id as the category ID
                    // and assign sequential IDs for each feature
                    const featuresWithIds = categoryData.details.map((detail, index) => ({
                        id: `${featureTitle.id}-${index}`, // Temporary ID
                        category: categoryName,
                        feature: detail,
                        // Store the actual feature ID if available
                        actualId: null // Will be populated when editing
                    }));
    ...
    // --- UPDATED: Edit multiple features with correct payload ---
    const editFeatures = async (categoryId, featuresArray) => {
        ...
        try {
            ...
            // IMPORTANT: We need to update each feature individually
            // The categoryId is the feature title ID
            // Each feature in the array should be updated with the same categoryId
            // but we need to handle the case where the feature might have a different ID
            
            // For each feature, we need to use the same category ID
            // because all features belong to the same category
            for (const feature of validFeatures) {
                ...
                // Use the category ID for all features
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (sectionType === 'hero') {
                ...
                // About section - check if we're editing an existing category
                if (editId) {
                    // Edit all features in the category
        ...
        <div style={styles.container}>
            ...
            {showModal && (
                <div style={styles.modalOverlay} onClick={() => { setShowModal(false); resetForm(); }}>
                    <div style={styles.modalCard} onClick={e => e.stopPropagation()}>
                        <div style={styles.modalHeader}>
                            <h5 style={{ margin: 0, fontWeight: '600' }}>
                                {editId && sectionType === 'about' ? '✏️ Edit Features Category' : 
                                 editId ? '✏️ Edit Content' : '✨ Add New Content'}
                            </h5>
                            <button 
                                onClick={() => { setShowModal(false); resetForm(); }}
            ...
            {featureDeleteConfirm && (
                <div style={styles.modalOverlay} onClick={() => setFeatureDeleteConfirm(null)}>
                    <div style={{...styles.modalCard, width: '400px'}} onClick={e => e.stopPropagation()}>
                        <div style={styles.modalHeader}>
                            <h5 style={{ margin: 0, fontWeight: '600' }}>Confirm Delete Category</h5>
                            <button onClick={() => setFeatureDeleteConfirm(null)} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer' }}>×</button>
                        </div>
                        <div style={styles.modalBody}>
                            <div style={{ textAlign: 'center' }}>
