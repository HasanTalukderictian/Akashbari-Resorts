...
const AdminSister = ({ theme: propsTheme }) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                const response = await axios.post(
                    `${API_BASE_URL}/partners/${editingId}`,
                    formDataToSend,
                    {
                        headers: {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this affiliate?')) {
            try {
                const response = await axios.delete(`${API_BASE_URL}/partners/${id}`);
                if (response.data.success) {
                    await fetchAffiliates();
                }
            } catch (error) {
                                        ...
                                        {affiliates.map((affiliate) => (
                                            <tr key={affiliate.id}>
                                                <td style={{ padding: '12px' }}>
                                                    <img 
                                                        src={affiliate.display_image_url}
                                                        alt={affiliate.title}
