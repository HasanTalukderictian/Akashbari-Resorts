...
const STAT_CARDS = [
    { key: 'totalUsers', label: 'Total Users', icon: 'bi-people-fill' },
    { key: 'totalEvents', label: 'Total Events', icon: 'bi-calendar-event-fill' },
    { key: 'totalGallery', label: 'Gallery Images', icon: 'bi-images' },
    { key: 'totalBlogs', label: 'Blog Posts', icon: 'bi-newspaper' },
    { key: 'totalTestimonials', label: 'Testimonials', icon: 'bi-star-fill' },
    { key: 'totalNotices', label: 'Notices', icon: 'bi-megaphone-fill' },
    { key: 'totalLuxuryItems', label: 'Luxury Items', icon: 'bi-gem' },
    { key: 'totalInvestmentBenifit', label: 'Investment Benefits', icon: 'bi-piggy-bank-fill' }
];

const RECENT_ACTIVITIES = [
    { id: 1, action: 'New user registered', time: '2 minutes ago', icon: 'bi-person-plus-fill' },
    { id: 2, action: 'New blog post published', time: '1 hour ago', icon: 'bi-newspaper' },
    { id: 3, action: 'New gallery image added', time: '3 hours ago', icon: 'bi-images' },
    { id: 4, action: 'New testimonial received', time: '5 hours ago', icon: 'bi-star-fill' },
    { id: 5, action: 'Event booking completed', time: '1 day ago', icon: 'bi-calendar-check-fill' }
];

const VIEW_COMPONENTS = {
    users: Users,
    owner: OwnerSection,
    benefit: OwnerBenefit,
    investment: Investment,
    notices: NoticeSection,
    gallery: GallerySection,
    events: Events,
    testimonials: Testominal,
    luxury: LuxurySection,
    'investment-benefits': InvestmentBenefit,
    blogs: BlogSection
};
...
const Dashbord = () => {
    ...
    const [isDarkMode, setIsDarkMode] = useState(false);
    const [dashboardStats, setDashboardStats] = useState({
        totalUsers: 0, totalEvents: 0, totalGallery: 0, totalBlogs: 0,
        totalTestimonials: 0, totalNotices: 0, totalLuxuryItems: 0, totalInvestmentBenifit: 0
    });
    const [loading, setLoading] = useState(true);
    const [authError, setAuthError] = useState(null);
    const [activeView, setActiveView] = useState('dashboard');
    ...
    const theme = {
        ...
        sidebarText: isDarkMode ? '#a3a3a3' : '#3e4b5b',
        gridColor: isDarkMode ? '#2b2b2b' : '#f5f5f5'
    };
    ...
    const fetchDashboardStats = async () => {
        ...
        try {
            const headers = getAuthHeaders();
            const safeGet = (url) => axios.get(url, { headers }).catch(err => {
                if (err.response?.status === 401) handleApiError(err);
                return { data: { data: { total: 0 }, length: 0 } };
            });

            const [usersRes, eventsRes, galleryRes, blogsRes, testimonialsRes, noticesRes, luxuryRes, investmentBenefitRes] =
                await Promise.all([
                    safeGet(`${API_BASE_URL}/users`),
                    safeGet(`${API_BASE_URL}/events/all`),
                    safeGet(`${API_BASE_URL}/gallery`),
                    safeGet(`${API_BASE_URL}/blogs`),
                    safeGet(`${API_BASE_URL}/get-testimonials`),
                    safeGet(`${API_BASE_URL}/notices`),
                    safeGet(`${API_BASE_URL}/luxury-items`),
                    safeGet(`${API_BASE_URL}/get-investment-benefits`)
                ]);

            const count = (res) => res.data?.data?.total || res.data?.data?.length || res.data?.length || 0;

            setDashboardStats({
                totalUsers: count(usersRes),
                totalEvents: count(eventsRes),
                totalGallery: count(galleryRes),
                totalBlogs: count(blogsRes),
                totalTestimonials: count(testimonialsRes),
                totalNotices: count(noticesRes),
                totalLuxuryItems: count(luxuryRes),
                totalInvestmentBenifit: count(investmentBenefitRes)
            });
        } catch (error) {
            console.error('Error fetching dashboard stats:', error);
            if (!handleApiError(error)) setAuthError('Failed to fetch dashboard data. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        const path = location.pathname.replace('/', '');
        setActiveView(path && path !== 'dashboard' ? path : 'dashboard');
    }, [location]);
    ...
    const styles = {
        container: { backgroundColor: theme.bg, minHeight: '100vh', color: theme.text },
        sidebar: {
            width: isCollapsed ? '80px' : '260px',
            backgroundColor: theme.card,
            height: '100vh',
        ...
        mainArea: { height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' },
        contentScroll: { flexGrow: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column' },
        statCard: { backgroundColor: theme.card, borderRadius: '16px', padding: '20px', border: `1px solid ${theme.border}` },
        statValue: { fontSize: '26px', fontWeight: 700, marginBottom: '4px', color: theme.text },
        statLabel: { fontSize: '13px', color: theme.textLight },
        activityItem: { display: 'flex', alignItems: 'center', padding: '12px', borderBottom: `1px solid ${theme.border}` },
        alert: { padding: '12px 20px', border: `1px solid ${theme.border}`, backgroundColor: theme.card, color: theme.text, borderRadius: '8px', marginBottom: '20px', fontWeight: 500 }
    };
    ...
    const renderContent = () => {
        ...
        if (ViewComponent) return <ViewComponent theme={theme} />;

        return (
            <div>
                {/* Welcome Section */}
                <div className="mb-4 p-4 rounded-4 d-flex justify-content-between align-items-center" style={{ backgroundColor: accent, color: accentOn }}>
                    <div>
                        <h2 className="mb-2">Welcome to Akashbari Resort Dashboard</h2>
                        <p className="mb-0 opacity-75">Manage your resort's content, users, and analytics all in one place</p>
                    </div>
                    <i className="bi bi-house-door-fill" style={{ fontSize: '44px' }}></i>
                </div>
                    ...
                    {STAT_CARDS.map(stat => (
                        <div className="col-xl-3 col-lg-4 col-md-6" key={stat.key}>
                            <div className="stat-card" style={styles.statCard}>
                                <div className="d-flex justify-content-between align-items-center">
                                    <div>
                                        <div style={styles.statValue}>{loading ? '...' : dashboardStats[stat.key]}</div>
                                        <div style={styles.statLabel}>{stat.label}</div>
                                    </div>
                                    <div style={{
                                        width: '48px', height: '48px', borderRadius: '12px',
                                        border: `1px solid ${theme.border}`, display: 'flex',
                                        alignItems: 'center', justifyContent: 'center', fontSize: '20px', color: theme.text
                                    }}>
                ...
                <div className="row g-4">
                    {/* Recent Activities */}
                    <div className="col-lg-6">
                        <div className="card border-0 rounded-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
                            <div className="card-header bg-transparent border-0 pt-4 px-4">
                                <h5 className="fw-bold mb-0" style={{ color: theme.text }}>Recent Activities</h5>
                            </div>
                            <div className="card-body p-0">
                                {RECENT_ACTIVITIES.map(activity => (
                                    <div key={activity.id} className="activity-item" style={styles.activityItem}>
                                        <div style={{
                                            ...
                                            border: `1px solid ${theme.border}`, display: 'flex',
                                            alignItems: 'center', justifyContent: 'center', marginRight: '12px', color: theme.text
                                        }}>
                                            <i className={`bi ${activity.icon}`}></i>
                                        </div>
                                        <div className="flex-grow-1">
                                            <div className="fw-semibold" style={{ color: theme.text }}>{activity.action}</div>
                                            <div style={{ fontSize: '12px', color: theme.textLight }}>{activity.time}</div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="col-lg-6">
                        <div className="card border-0 rounded-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
                            <div className="card-header bg-transparent border-0 pt-4 px-4">
                                <h5 className="fw-bold mb-0" style={{ color: theme.text }}>
                                    <i className="bi bi-lightning-charge-fill me-2"></i>Quick Actions
                                </h5>
                                <p className="small mb-0 mt-1" style={{ color: theme.textLight }}>Quick access to important features</p>
                            </div>
                            <div className="card-body">
                                <div className="d-flex justify-content-between align-items-center p-3 rounded-3" style={{ border: `1px solid ${theme.border}` }}>
                                    <div>
                                        <span style={{ fontSize: '14px', color: theme.textLight }}>Today's Views</span>
                                        <h4 className="mb-0 fw-bold" style={{ color: theme.text }}>1,234</h4>
                                    </div>
                                    <div>
                                        <span style={{ fontSize: '14px', color: theme.textLight }}>Total Clicks</span>
                                        <h4 className="mb-0 fw-bold" style={{ color: theme.text }}>5.6K</h4>
                                    </div>
                                    <i className="bi bi-bar-chart-fill" style={{ fontSize: '28px', color: theme.text }}></i>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div style={styles.container} className="container-fluid p-0">
            <style>{`
                ...
                .activity-item:hover { background: ${theme.bg}; transform: translateX(4px); }
                .activity-item { transition: all 0.2s ease; }
            `}</style>
            <div className="d-flex">
                <Sidebar theme={theme} isCollapsed={isCollapsed} activeView={activeView} setActiveView={setActiveView} styles={styles} />
                ...
                <div style={styles.mainArea} className="flex-grow-1">
                    ...
                    <div style={styles.contentScroll}>
                        <div className="flex-grow-1">
                            <h4 className="mb-4 text-capitalize d-flex align-items-center fw-bold" style={{ color: theme.text }}>
                                <span style={{ backgroundColor: accent, color: accentOn, padding: '8px', borderRadius: '8px', marginRight: '10px' }}>
                                    <i className="bi bi-grid-fill"></i>
                                </span>
                                {activeView === 'dashboard' ? 'Dashboard Overview' : activeView.replace('-', ' ')}
                            </h4>
